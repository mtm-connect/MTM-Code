{{-- resources/views/pdf/items/threepiece.blade.php --}}
@php
use App\Support\PdfAsset;

/* ===== Resolve the selected item ===== */
$selectedItem = null; $type = 'three_piece';
if (!empty($selected_threepiece)) { $selectedItem = $selected_threepiece; }
...
/**
 * Safe local-image resolver (same pattern as jacket.blade.php):
...
/* IMPORTANT: match jacket.blade.php – JPG files + dedicated folder */
...
/* IMPORTANT: folder changed to buttons_contrast_jpg to match jacket PDF */
...
/** Waistcoat **/
$Waistcoat_Tape_Data                = $img('waistcoat_types',       $Waistcoat_Tape_Images,                $S->waistcoat_type ?? null);

/* Helper for codes – same idea as jacket */
...
<head>
<meta charset="utf-8">
<title>Three Piece – Order #{{ $ordNo }}</title>
<style>
...
<div class="section">
  ...
  <table>
    <thead><tr><th>Code Field</th><th>Value</th></tr></thead>
    <tbody>
      <tr><td>Code Waistcoat</td><td>{{ $code($selectedItem->code_waistcoat ?? '') }}</td></tr>
      <tr><td>Code Waistcoat Buttons</td><td>{{ $code($selectedItem->code_waistcoat_buttons ?? '') }}</td></tr>
    </tbody>
  </table>

  <table class="grid4" style="margin-top:8px;">
    <tr>
      <td class="cell">
        <div class="img-box">
          @if($Waistcoat_Tape_Data)<img src="{{ $Waistcoat_Tape_Data }}" alt="Waistcoat Type">@endif
        </div>
        <div>Type: {{ $selectedItem->waistcoat_type ?? '' }}</div>
      </td>
      <td class="cell"></td>
      <td class="cell"></td>
      <td class="cell"></td>
