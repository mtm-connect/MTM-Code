{{-- resources/views/pdf/items/overcoat.blade.php --}}
@php
use App\Support\PdfAsset; // still fine even if we don't use it

/* ===== Resolve the selected item ===== */
$selectedItem = null; $type = 'overcoat';
if (!empty($selectedovercoat)) { $selectedItem = $selectedovercoat; }
...
/* ===== Image maps – KEEP IMAGES EXACTLY THE SAME as jacket PDF/view ===== */
$Jacket_Type_Images = [
...
// NOTE: attributes are now overcoat_* but folders/filenames are unchanged
$Jacket_Type_Data           = $img('jacket_type',             $Jacket_Type_Images,                    $S->overcoat_type ?? null);
$Jacket_Construction_Data   = $img('canvas',                  $Jacket_Construction_Images,            $S->overcoat_construction ?? null);
$Jacket_LapelType_Data      = $img('lapel_type',              $Jacket_LapelType_Images,               $S->overcoat_lapel_type ?? null);
$Jacket_SatinType_Data      = $img('satin_lapel',             $Jacket_SatinType_Images,               $S->overcoat_satin_lapel ?? null);
$handstitch_Data            = $img('handstitch',              $handstitch_Images,                     $S->overcoat_hand_stitch ?? null);
$lapel_width_Data           = $img('lapel_width',             $lapel_width_Images,                    $S->overcoat_lapel_width ?? null);
$lapel_functional_Data      = $img('lapel_functional_button', $lapel_functional_button_Images,        $S->overcoat_lapel_functional_button ?? null);
$jacket_sleeve_buttons_Data = $img('sleeve_buttons',          $jacket_sleeve_buttons_Images,          $S->overcoat_sleeve_buttons ?? null);
$jacket_functional_Data     = $img('functional_buttons',      $jacket_functional_buttons_Images,      $S->overcoat_functional_buttons ?? null);

// IMPORTANT: your jacket file had 'buttons_contrast_jpg' here (likely wrong).
// Your view folder is 'buttons_contrast_colour' – keeping it consistent for overcoat PDF:
$jacket_buttons_colour_last_Data
    = $img('buttons_contrast_colour', $jacket_buttons_colour_on_last_button_hole_Images, $S->overcoat_buttons_colour_on_last_button_hole ?? null);

$jacket_lining_Data         = $img('lining',                  $jacket_lining_Images,                  $S->overcoat_lining ?? null);
$jacket_pockets_Data        = $img('pockets',                 $jacket_pockets_Images,                 $S->overcoat_pockets ?? null);
$jacket_pockets_flap_Data   = $img('pockets_with_flap',       $jacket_pockets_with_flap_Images,       $S->overcoat_pockets_with_flap ?? null);
$jacket_italian_Data        = $img('italian_pockets',         $jacket_italian_pockets_Images,         $S->overcoat_italian_pockets ?? null);
$jacket_patch_Data          = $img('patch_pockets',           $jacket_patch_pockets_Images,           $S->overcoat_patch_pockets ?? null);
$jacket_satin_piping_Data   = $img('pockets_satin_piping',    $jacket_pockets_satin_piping_Images,    $S->overcoat_pockets_satin_piping ?? null);
$jacket_chest_Data          = $img('chest_pocket',            $jacket_chest_pocket_type_Images,       $S->overcoat_chest_pocket_type ?? null);
$jacket_vents_Data          = $img('vents',                   $jacket_vents_Images,                   $S->overcoat_vents ?? null);
...
<head>
<meta charset="utf-8">
<title>Overcoat – Order #{{ $ordNo }}</title>
<style>
...
{{-- Overcoat Codes --}}
<div class="section">
  <h2>Overcoat Codes</h2>
  <table>
    <thead><tr><th>Code Field</th><th>Value</th></tr></thead>
    <tbody>
      <tr><td>Overcoat Fabric</td><td>{{ $code($S->code_overcoat ?? '') }}</td></tr>

      @if(($S->overcoat_lining ?? '') !== 'No Lining')
      <tr><td>Overcoat Lining</td><td>{{ $code($S->code_overcoat_lining ?? '') }}</td></tr>
      @endif

      <tr><td>Overcoat Buttons</td><td>{{ $code($S->code_overcoat_button ?? '') }}</td></tr>

      @if(($S->overcoat_satin_lapel ?? '') !== 'No Satin Lapel')
      <tr><td>Satin Lapel</td><td>{{ $code($S->code_satin_lapel ?? '') }}</td></tr>
      @endif

      @if(($S->overcoat_buttons_colour_on_last_button_hole ?? '') === 'Yes')
...
{{-- Overcoat Options – 4-column grid (same layout/images) --}}
<div class="section">
  <h2>Overcoat Construction</h2>
  <table class="grid4">
    <tr>
      <td class="cell">
        <div class="img-box">
          @if($Jacket_Type_Data)<img src="{{ $Jacket_Type_Data }}" alt="Overcoat Type">@endif
        </div>
        <div>Type: {{ $S->overcoat_type ?? '' }}</div>
        <div class="tag">Fabric: {{ $code($S->code_overcoat ?? '') }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Construction: {{ $S->overcoat_construction ?? '' }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Lapel: {{ $S->overcoat_lapel_type ?? '' }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Satin Lapel: {{ $S->overcoat_satin_lapel ?? '' }}</div>
        @if(($S->overcoat_satin_lapel ?? '') !== 'No Satin Lapel' && !empty($S->code_satin_lapel ?? ''))
    ...
    <tr>
      <td class="cell">
        ...
        <div>Lapel Hand Stitch: {{ $S->overcoat_hand_stitch ?? '' }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Lapel Width: {{ $S->overcoat_lapel_width ?? '' }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Lapel Functional Button: {{ $S->overcoat_lapel_functional_button ?? '' }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Sleeve Buttons: {{ $S->overcoat_sleeve_buttons ?? '' }}</div>
        <div class="tag">Buttons: {{ $code($S->code_overcoat_button ?? '') }}</div>
      </td>
    </tr>
    ...
    <tr>
      <td class="cell">
        ...
        <div>Functional Buttons: {{ $S->overcoat_functional_buttons ?? '' }}</div>
      </td>

      <td class="cell">
        <div class="img-box">
          @if($jacket_lining_Data)<img src="{{ $jacket_lining_Data }}" alt="Lining">@endif
        </div>
        <div>Lining: {{ $S->overcoat_lining ?? '' }}</div>
        @if(($S->overcoat_lining ?? '') !== 'No Lining' && !empty($S->code_overcoat_lining ?? ''))
          <div class="tag">Lining: {{ $code($S->code_overcoat_lining) }}</div>
        @endif
      </td>

      <td class="cell">
        <div class="img-box">
          @if($jacket_pockets_Data)<img src="{{ $jacket_pockets_Data }}" alt="Pockets">@endif
        </div>
        <div>Pockets: {{ $S->overcoat_pockets ?? '' }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Pockets with Flap: {{ $S->overcoat_pockets_with_flap ?? '' }}</div>
      </td>
    </tr>
    ...
    <tr>
      <td class="cell">
        ...
        <div>Italian Pockets: {{ $S->overcoat_italian_pockets ?? '' }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Patch Pockets: {{ $S->overcoat_patch_pockets ?? '' }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Satin Piping: {{ $S->overcoat_pockets_satin_piping ?? '' }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Chest Pocket: {{ $S->overcoat_chest_pocket_type ?? '' }}</div>
      </td>
    </tr>
    ...
    <tr>
      <td class="cell">
        ...
        <div>Vents: {{ $S->overcoat_vents ?? '' }}</div>
      </td>
      ...
      <td class="cell">
        ...
        <div>Colour on Last Button Hole: {{ $S->overcoat_buttons_colour_on_last_button_hole ?? '' }}</div>
        @if(($S->overcoat_buttons_colour_on_last_button_hole ?? '') === 'Yes')
